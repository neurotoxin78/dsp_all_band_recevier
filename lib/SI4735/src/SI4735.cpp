...
int16_t SI4735::getDeviceI2CAddress(uint8_t resetPin)
{
    ...
    Wire.begin(GPIO_NUM_4, GPIO_NUM_0);
...
void SI4735::radioPowerUp(void)
{
    ...
    waitToSend();
    vTaskDelay(maxDelayAfterPouwerUp);
...
void SI4735::setFrequency(uint16_t freq)
{
    ...
    vTaskDelay(maxDelaySetFrequency); // For some reason I need to delay here.
}
...
void SI4735::seekStation(uint8_t SEEKUP, uint8_t WRAP)
{
    ...
    Wire.endTransmission();
    vTaskDelay(MAX_DELAY_AFTER_SET_FREQUENCY << 2);
}
...
{
    seekStation(1, 1);
    vTaskDelay(maxDelaySetFrequency);
    getFrequency();
}
...
void SI4735::setFrequencyNBFM(uint16_t freq)
{
    ...
    waitToSend();                // Wait for the si473x is ready.
    currentWorkFrequency = freq; // check it
    vTaskDelay(250); // For some reason I need to delay here.
}
